 ...
 * 拦截器 用于校验权限
 */
public final class TestAuthInterceptor implements ScxMappingInterceptor {
    ...
    public void preHandle(RoutingContext context, ScxMappingHandler scxMappingHandler) {
        ...
        if (p.checkedLogin) {
            //先获取登录的用户
            var currentUser = TestAuth.getLoginUser(context);
            //如果用户为空 则执行未登录处理器
            if (currentUser == null) {
                throw new UnauthorizedException();
            } else if (p.checkedPerms && !currentUser.isAdmin && !TestAuth.getPerms(currentUser).contains(p.permStr)) {
